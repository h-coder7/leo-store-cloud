...
import { getSection, updateSection } from '@/app/actions/products';
    ...
    return (
        <div className="p-8 max-w-2xl mx-auto" dir="rtl">
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 dark:text-white">تعديل القسم</h1>
                    <p className="text-slate-500 mt-1">تعديل بيانات القسم: {section.name}</p>
                </div>
                <Link
                    href="/admin/sections"
            ...
            <div className="bg-white dark:bg-slate-800 rounded-3xl shadow p-8">
                <form action={updateSectionWithId} className="space-y-6">
                    {/* Name */}
                    <div>
                    ...
                    <div>
                        ...
                        {section.image_url && (
                            <div className="mb-4 relative w-32 aspect-video rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700">
                                <img
                                    src={section.image_url}
                                    alt="Current"
                                    className="w-full h-full object-cover"
                                />
                                <input type="hidden" name="current_image_url" value={section.image_url} />
                            </div>
                        )}
                        ...
                        />
                        {section.image_url && (
                            <p className="text-[10px] text-slate-500 mt-1">اختر صورة جديدة لاستبدال الصورة الحالية، أو اتركها كما هي.</p>
                        )}
                    </div>
                    ...
                    <div className="pt-4 flex justify-end gap-4">
                        ...
                        <SectionSubmitButton label="✓ تحديث القسم" />
                    </div>
                </form>
            </div>
        </div>
