...
import { toggleOfferStatus, deleteOffer } from '@/app/actions/offers';
...
export default function OfferActions({ id, isActive }: OfferActionsProps) {
    const [isPending, startTransition] = useTransition();

    const handleToggle = () => {
        startTransition(async () => {
            const res = await toggleOfferStatus(id, isActive);
            if (res.success) {
    ...
    return (
        <div className="p-4 flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
                ...
                <Link
                    href={`/admin/offers/edit/${id}`}
                    className="p-2 text-blue-500 bg-blue-50 dark:bg-blue-500/10 hover:bg-blue-100 rounded-xl transition-colors"
                    title="تعديل"
                >
